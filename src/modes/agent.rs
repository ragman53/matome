...
This is a documentation workspace exported by matome.

## Navigation

1. Start with `manifest.json` to understand the workspace structure
2. Check `token_budget.json` for context window limits
3. Code examples are authoritative - never rewrite them
...
This workspace was generated with:
- Max tokens: {}
...
"#,
            self.workspace_name, self.max_tokens
        );
