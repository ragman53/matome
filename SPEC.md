# matome - Specification (v0.2.0 Draft)
...
>
> **📌 バージョン戦略**:
> - **v0.1.0**: 現行プロトタイプ（フラットな articles テーブル、翻訳機能中心）
> - **v0.2.0**: 新機能 направление（3モードアーキテクチャ、階層構造、AI Agent対応）
...
| バージョン | 状態 | 説明 |
|-----------|------|------|
| **v0.1.0** | ✅ 完成 | 現行プロトタイプ。フラットな articles テーブル、翻訳機能中心 |
| **v0.2.0** | 🔄 開発中 | この仕様書の方向性。3モードアーキテクチャ、階層構造、AI Agent対応 |
| **v1.0.0** | 📋 目標 | 完全リリース。全ての基本機能が安定、板書なしにユーザーが利用可能 |

### 2.1 v0.2.0 Goals

> **現在の方向性を探る段階** - 以下の機能を実験的に実装し、ユーザーのフィードバックを収集する
...
| 項目 | 目標値 | 対策 |
|------|--------|------|
| **クロール速度** | 100ページ/分 | 並列制御セマフォ + HTTP/2 + ETag対応 |
...
| **バイナリサイズ** | ≤ 50MB (--release --strip) | 依存関係最小化 |
